...
function player_create(_spriteIdle, _spriteRun, _spriteJump, _spriteMelee, _spriteFire, _spriteDash) 
{
	//initialize physics.
	...
	//The skill currently being selected from global.playerSkills
	skillIndex = 0;
...
function player_draw_gui() 
{
	...
	//Location name..
	draw_set_color(c_gray);
	draw_text(CAMERA_WIDTH - 5, 27, "Location: " + global.roomName);

	draw_set_color(c_white);
	draw_text(CAMERA_WIDTH - 5, 25, "Location: " + global.roomName);
	...
	if (array_length(global.playerSkills) > 0)
	{
		draw_text(5, 65, "Skill: " + global.playerSkills[skillIndex].skillName);
	}
...
function player_skill() 
{
	var _skill = global.playerSkills[skillIndex];
