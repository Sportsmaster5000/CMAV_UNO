...
{
	//_vsp = round(_vsp); This fixes it but makes the player fly
	// bug or feature??
	...
	for (var _i = 0; (abs(_i) < abs(_vsp)) || (place_meeting(x, y + _i, BLOCK)); _i += sign(_vsp))
	{
	    ...
	    if (_vsp >= 0)
	    {	
			if (collision_line(bbox_left, bbox_bottom + _i + 1, bbox_right, bbox_bottom + _i+ 1, obj_oneWay, true, false))
			//if (collision_point(x, bbox_bottom + _i + 1, obj_oneWay, true, false))
			{
	                y += _i;
	                return 0;
			}
			/*
			if (position_meeting(x, bbox_bottom +_i + 1, obj_oneWay))
			{
			}
	        if (bottom(bbox_bottom + 1 + (_i), obj_oneWay))
	        {
	                y += _i;
	                return 0;
	        }
